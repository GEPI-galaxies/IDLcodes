...
; Image profile
img=READFITS(SUBCOMP_FILE, EXTEN_NO=0,/SILENT) ;;;; warning : regarder ce que contient subcomps pour les indices dans les boucles !! ici : k=0 img k=1 comp1 k=2 comp2 k=3 rien
;img=img-sky
hsize = (SIZE(img))[1]
vsize = (SIZE(img))[2]
angle_major=PA-90.
...
ENDFOR
;stop
model_profile_major[*,2]=0.0
model_profile_minor[*,2]=0.0
...
model_profile_major[*,0]=model_profile_major[*,0]*1e10
model_profile_major[*,1]=model_profile_major[*,1]*1e10
model_profile_major[*,2]=model_profile_major[*,2]*1e10

model_profile_minor[*,0]=model_profile_minor[*,0]*1e10
model_profile_minor[*,1]=model_profile_minor[*,1]*1e10
model_profile_minor[*,2]=model_profile_minor[*,2]*1e10

tot_profile_major = TOTAL(model_profile_major,2)  
tot_profile_minor = TOTAL(model_profile_minor,2)

;tot_profile_major=tot_profile_major*1e10
;tot_profile_minor=tot_profile_minor*1e10
;stop

;mag_img_profile_major=-2.5*alog10(img_profile_major) + zp
;mag_img_profile_minor=-2.5*alog10(img_profile_minor) + zp

mag_img_profile_major=alog10(img_profile_major)
mag_img_profile_minor=alog10(img_profile_minor)

mag_model_profile_major=model_profile_major
mag_model_profile_minor=model_profile_minor

;FOR k=0,Ncomp-1 DO BEGIN
;   mag_model_profile_major[*,k]=-2.5*alog10(model_profile_major[*,k]) + zp
;   mag_model_profile_minor[*,k]=-2.5*alog10(model_profile_minor[*,k]) + zp 
;ENDFOR
;mag_tot_profile_major = -2.5*alog10(tot_profile_major) + zp
;mag_tot_profile_minor = -2.5*alog10(tot_profile_minor) + zp

FOR k=0,Ncomp-1 DO BEGIN
   mag_model_profile_major[*,k]=alog10(model_profile_major[*,k])
   mag_model_profile_minor[*,k]=alog10(model_profile_minor[*,k])
ENDFOR
mag_tot_profile_major = alog10(tot_profile_major)
mag_tot_profile_minor = alog10(tot_profile_minor)
...
ylimits_flux=[(min(img_profile_major)*0.9)-1e9,max(img_profile_major)*1.1]
...
mag_min=min(mag_img_profile_major[where(finite(mag_img_profile_major))])
mag_max=max(mag_img_profile_major[where(finite(mag_img_profile_major))])

ylimits_mag=[mag_min-0.1,mag_max+0.1]
;stop
...
; !x.margin=3.2
; !y.margin=2.    
cgPLOT,xaxis,img_profile_major,xtit='Major axis (kpc)',ytit=textoidl('dmass (Mo/kpc2)'),thick=3,xs=1,xr=xlimits,yr=ylimits_flux,ys=1,xthick=2,ythick=2,charsize=1,color='black',charthick=2,aspect=1.,Position=pos[*,0]
FOR k=0,Ncomp-1 DO BEGIN
...
cgPLOT,xaxis,mag_img_profile_major,xtit='Major axis (kpc)',ytit=textoidl('log(dmass) (Mo/kpc2)'),thick=3,xs=1,xr=xlimits,yr=ylimits_mag,ys=1,xthick=2,ythick=2,charsize=1,color='black',charthick=2,aspect=1.,Position=pos[*,1], /NoErase
FOR k=0,Ncomp-1 DO BEGIN
...
cgPLOT,xaxis,img_profile_minor,xtit='Minor axis (kpc)',ytit=textoidl('dmass (Mo/kpc2)'),thick=3,xs=1,xr=xlimits,yr=ylimits_flux,ys=1,xthick=2,ythick=2,charsize=1,color='black',charthick=2,aspect=1.,Position=pos[*,2] , /NoErase
FOR k=0,Ncomp-1 DO BEGIN
...
cgPLOT,xaxis,mag_img_profile_minor,xtit='Minor axis (kpc)',ytit=textoidl('log(dmass) (Mo/kpc2)'),thick=3,xs=1,xr=xlimits,yr=ylimits_mag,ys=1,xthick=2,ythick=2,charsize=1,color='black',charthick=2,aspect=1.,Position=pos[*,3], /NoErase
FOR k=0,Ncomp-1 DO BEGIN
...
;;; LEGEND
IF ncomp GE 2 THEN cgLEGEND,titles=['observation',Components[*].type,'total model'],colors=['black',Components[*].Color,'red'],length=0.02,linestyles=[0,REPLICATE(2,ncomp),0],LOCATION=[0.32,0.95],thick=2,charsize=1.,vspace=1. ELSE cgLEGEND,titles=['observation',Components[*].type],colors=['black',Components[*].Color],length=0.02,linestyles=[0,REPLICATE(2,ncomp)],LOCATION=[0.32,0.95],thick=2,charsize=1.,vspace=1.
