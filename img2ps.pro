...
PRO img2ps ,img, XY_max_scale, Z_range ,XY_label, SAVE_EPS=SAVE_EPS, SAVE_PNG=SAVE_PNG, GREY=GREY, INVERSE=INVERSE, LOG=LOG, Z_label=Z_label
  ...
  value_range=minmax(img,/NAN)
  img_scale = BytScl(img, Min=value_range[0], Max=value_range[1], /NAN)
  
   IF KEYWORD_SET(LOG) then begin 
   img_scale=alogscale(img,/auto) 
   value_range=alog10(value_range)
 ...
 ;  img_scale = BytScl(img, Min=value_range[0], Max=value_range[1], /NAN)
   ...
   value_range=REVERSE(value_range)
   ENDIF
   ...
   IF KEYWORD_SET(SAVE_EPS) then cgPS_Open, SAVE_EPS, /inches, portrait=0,  xsize=6, ysize=3.75,$ 
        xoffset=1.25, yoffset=3.62, /nomatch ,encapsulated=1.
   
   cgDisplay, size_display[0], size_display[1]
   ...
   IF KEYWORD_SET(Z_label) THEN cgColorbar, Position=[0.875, 0.120, 0.92, 0.95], Title=Z_label,/Reverse, $
        NColors=254, Bottom=1,Ticklen=-0.25, /Top, /VERTICAL,  Range=[value_range[0], value_range[1]],$
       TLocation='RIGHT'       
   ENDIF
