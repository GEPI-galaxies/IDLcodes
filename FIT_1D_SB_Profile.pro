...
cgplot, sma,Int,psym=2,/ylog, yrange=[0.00001, max(Int)*1.2],xrange=[0,120]
...
for i=0, n_component -1 do begin
	...
	CASE Components[i].type OF
		'sersic' : BEGIN
				   ...
				   Components[i].PA = PA[round(Fit_results[1])]	
				   Components[i].b_a = 1-E[round(Fit_results[1])]					   
...
IF KEYWORD_SET(PRINT_LOG) then begin 
	...
	printf,lun,format='(a)',' 1) '+ strn(xc[0], format='(f6.2)' ) +" "+ strn(yc[0], format='(f6.2)' ) +'  1  1        #  position x, y   '                  
