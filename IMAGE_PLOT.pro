...
READCOL,path+'/KMOS/KMOS3D_Wisnioski2014_z1.cat',id,dec,ra,redshift,FORMAT='(a,f,f,f)'
...
print,id[i]
	loadct,0
	result = FILE_TEST('images/'+id[i]+'_V.fits')
	IF result then begin 
	image_V=readfits('images/'+id[i]+'_V.fits',header)
	...
	result = FILE_TEST('images/'+id[i]+'_I.fits')
	IF result then begin 
	image_I=readfits('images/'+id[i]+'_I.fits',header)
	...
	result = FILE_TEST('images/'+id[i]+'_J.fits')
	IF result then begin 
	image_J=readfits('images/'+id[i]+'_J.fits',header)
	...
	result = FILE_TEST('images/'+id[i]+'_H.fits')
	IF result then begin 
	image_H=readfits('images/'+id[i]+'_H.fits',header)
...
void = cgSnapshot(FILENAME='images/'+id[i]+'.png',/nodialog)
endfor
;!P.Multi = 0
